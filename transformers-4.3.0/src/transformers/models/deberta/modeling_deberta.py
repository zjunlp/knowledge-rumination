...
class DebertaIntermediate(nn.Module):
    def __init__(self, config, index):
        super().__init__()
        if type(config) != DebertaConfig and index in config.ffn_layer:
...
class DebertaLayer(nn.Module):
    def __init__(self, config, index):
        ...
        self.intermediate = DebertaIntermediate(config, index)
...
class DebertaEncoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.layer = nn.ModuleList([DebertaLayer(config, i) for i in range(config.num_hidden_layers)])
...
class DebertaModel(DebertaPreTrainedModel):
    def __init__(self, config):
        ...
        self.z_steps = 0
        self.config = config
        if type(config) != DebertaConfig and config.inject_concat:
            self.inject_concat = True
        else:
            self.inject_concat = False
        if type(config) != DebertaConfig and config.inject_ffn:
