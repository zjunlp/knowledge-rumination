...
from transformers import (TrainingArguments, HfArgumentParser, EvalPrediction, set_seed, Trainer,
                          RobertaTokenizer, DebertaTokenizer,
                          BartConfig, BartTokenizer,
                          T5Config, T5Tokenizer)
from model import RumiBART, RumiRoBERTa, RumiT5, RumiRobertaConfig, RumiDebertaConfig, RumiDeBERTa
...
MODEL_CLASSES = {
    'roberta': (RumiRobertaConfig, RumiRoBERTa, RobertaTokenizer),
    'deberta': (RumiDebertaConfig, RumiDeBERTa, DebertaTokenizer),
...
def main():
    ...
    train_dataset = (
        MultipleChoiceDataset(
            ...
            rumi_length=data_args.rumi_length,
    ...
    if training_args.do_train:
        trainer.train(
            resume_from_checkpoint=model_args.model_name_or_path if os.path.isdir(
